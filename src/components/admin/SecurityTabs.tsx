...
const TABS = [
  ...
  { href: "/admin/security/sessions", label: "내 세션" },
    ...
    <div className="flex gap-1 border-b border-neutral-200 mb-4">
      {TABS.map((t) => (
        <Link
          key={t.href}
          href={t.href}
          className={`px-3 py-2 text-sm ${active === t.href ? "border-b-2 border-indigo-600 text-indigo-600 font-medium" : "text-neutral-500 hover:text-neutral-800"}`}
        >
          {t.label}
        </Link>
      ))}
