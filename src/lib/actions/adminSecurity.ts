...
import { getClientInfo, destroyAdminSession } from "@/lib/session";
import { requireAdmin, requireSuperAdmin } from "@/lib/actions/adminAuth";
import { revalidatePath } from "next/cache";
import { redirect } from "next/navigation";
import QRCode from "qrcode";

export async function logAdminActivity(adminId: string, action: string, target?: string, detail?: string) {
  const { ip } = await getClientInfo().catch(() => ({ ip: undefined }));
  await prisma.adminActivityLog.create({ data: { adminId, action, target, detail, ip } });
}
...
export async function createAdminAction(_prev: ActionState, formData: FormData): Promise<ActionState> {
  ...
  const admin = await prisma.adminUser.create({ data: { loginId, passwordHash, name, role } });
...
export async function revokeAdminSessionAction(formData: FormData): Promise<void> {
  ...
  await prisma.adminSession.deleteMany({ where: { id: sessionId, adminId: admin.id } });
