...
import { INQUIRY_STATUS, PAYMENT_STATUS } from "@/lib/constants";

const NAV: { href: string; label: string }[] = [
  { href: "/admin", label: "대시보드" },
  { href: "/admin/orders", label: "주문 관리" },
  { href: "/admin/members", label: "회원 관리" },
  { href: "/admin/services", label: "서비스/가격 관리" },
    ...
    <div className="min-h-screen flex bg-neutral-100">
        ...
        <nav className="space-y-1 text-sm">
          {NAV.map((item) => (
            <Link key={item.href} href={item.href} className="flex items-center justify-between px-3 py-2 rounded-md hover:bg-neutral-800 hover:text-white">
              ...
              {badges[item.href] > 0 && (
                <span className="bg-red-500 text-white text-xs rounded-full px-1.5">{badges[item.href]}</span>
              )}
            </Link>
          ))}
        </nav>
      ...
      <div className="flex-1 min-w-0">
        <header className="h-14 bg-white border-b border-neutral-200 flex items-center justify-end gap-4 px-6 text-sm">
          <span className="text-neutral-500">{admin.name} ({admin.role})</span>
